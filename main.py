...
from stock_selector import ConceptBoardSelector
...
class StockAnalysisPipeline:
    ...
    def analyze_stock(self, code: str) -> Optional[AnalysisResult]:
        ...
        try:
            ...
            stock_name = self.stock_name_cache.get(code, "") or STOCK_NAME_MAP.get(
                code, ""
            )
        ...
        try:
            ...
            if result:
                ...
                if single_stock_notify and self.notifier.is_available():
                    try:
                        single_report = self.notifier.generate_single_stock_report(
                            result
                        )
...
def main() -> int:
    ...
    # === 启动 WebUI (如果启用) ===
    # 优先级: 命令行参数 > 配置文件
    start_webui = (args.webui or config.webui_enabled) and os.getenv(
        "GITHUB_ACTIONS"
    ) != "true"
