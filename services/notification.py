...
class NotificationService:
    ...
    def generate_screening_report(
        ...
        Args:
            screening_results: 选股结果列表
            report_date: 报告日期（默认今天）

        Returns:
            Markdown 格式的选股报告
        """
        from screeners.stock_screener import ScreeningResult
        ...
        # 标题
        report_lines = [
            f"# 🎯 {report_date} 全市场选股报告",
            "",
            f"> 共选出 **{len(screening_results)}** 只股票 | 报告生成时间：{datetime.now().strftime('%H:%M:%S')}",
            "",
            "---",
            "",
        ]
        ...
        sorted_results = sorted(
            screening_results,
            key=lambda x: x.tech_score,
            reverse=True
        )

        # 统计信息
        has_ai = sum(1 for r in screening_results if r.ai_result is not None)
        avg_tech_score = sum(r.tech_score for r in screening_results) / len(screening_results) if screening_results else 0
        avg_ai_score = sum(r.ai_result.sentiment_score for r in screening_results if r.ai_result) / has_ai if has_ai > 0 else 0

        # 统计操作建议
        if has_ai > 0:
            buy_count = sum(1 for r in screening_results if r.ai_result and r.ai_result.operation_advice in ['买入', '加仓', '强烈买入'])
            hold_count = sum(1 for r in screening_results if r.ai_result and r.ai_result.operation_advice in ['持有', '观望'])
            sell_count = sum(1 for r in screening_results if r.ai_result and r.ai_result.operation_advice in ['卖出', '减仓', '强烈卖出'])
        else:
        ...
        report_lines.extend([
            ...
            f"| 指标 | 数值 |",
            f"|------|------|",
            f"| 选出股票数 | **{len(screening_results)}** 只 |",
            f"| 平均技术评分 | **{avg_tech_score:.1f}** 分 |",
        ...
        for result in sorted_results:
            # 技术评分emoji
            if result.tech_score >= 80:
                tech_emoji = "💚"
            elif result.tech_score >= 65:
                tech_emoji = "🟢"
            elif result.tech_score >= 50:
                tech_emoji = "🟡"
            else:
                tech_emoji = "⚪"

            report_lines.extend([
                f"### {tech_emoji} {result.name} ({result.code})",
                "",
                f"**技术评分**: {result.tech_score:.1f} 分",
            ])

            # 技术面理由
            if result.tech_reasons:
                report_lines.extend([
                    "**技术面理由**:",
                ])
                for reason in result.tech_reasons:
                    report_lines.append(f"- {reason}")
                report_lines.append("")

            # AI分析结果
            if result.ai_result:
                ai_emoji = result.ai_result.get_emoji()
                report_lines.extend([
                    f"**AI分析**: {ai_emoji} {result.ai_result.operation_advice} | "
                    f"评分 {result.ai_result.sentiment_score} | {result.ai_result.trend_prediction}",
                    "",
                ])

                # 核心结论
                if result.ai_result.analysis_summary:
                    report_lines.extend([
                        f"**核心分析**: {result.ai_result.analysis_summary}",
                        "",
                    ])

                # 买入理由
                if result.ai_result.buy_reason:
                    report_lines.extend([
                        f"**操作理由**: {result.ai_result.buy_reason}",
                        "",
                    ])

                # 风险提示
                if result.ai_result.risk_warning:
                    report_lines.extend([
                        f"**⚠️ 风险提示**: {result.ai_result.risk_warning}",
                        "",
                    ])

            # 狙击点位
            if result.ai_result:
                sniper_points = result.ai_result.get_sniper_points()
                if sniper_points:
                    points = []
                    if sniper_points.get('ideal_buy'):
                        points.append(f"🎯 买点: {sniper_points['ideal_buy']}")
                    if sniper_points.get('stop_loss'):
                        points.append(f"🛑 止损: {sniper_points['stop_loss']}")
                    if sniper_points.get('take_profit'):
                        points.append(f"🎊 目标: {sniper_points['take_profit']}")
                    if points:
                        report_lines.extend([
                            "**操作点位**: " + " | ".join(points),
                            "",
        ...
        report_lines.extend([
            ...
            f"*报告生成时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*",
            "",
            "*注：选股结果仅供参考，不构成投资建议。投资有风险，入市需谨慎。*",
        ])

        return "\n".join(report_lines)

    def generate_wechat_screening_report(
        self,
        screening_results: List[Any],
        max_length: int = 3800,
        report_date: Optional[str] = None
        ...
        lines = [
            f"## 🎯 {report_date} 全市场选股",
            "",
            f"> 选出 {len(screening_results)} 只股票",
            "",
        ]
        ...
        for result in sorted_results:
            ...
            # 标题行
            lines.append(f"### {tech_emoji} {result.name}({result.code})")
            lines.append(f"**技术评分**: {result.tech_score:.1f}")

            # 技术面理由（最多3条）
            if result.tech_reasons:
                reasons_str = " | ".join(result.tech_reasons[:3])
                lines.append(f"**理由**: {reasons_str}")

            # AI分析结果
            if result.ai_result:
                ai_emoji = result.ai_result.get_emoji()
                lines.append(f"**AI**: {ai_emoji}{result.ai_result.operation_advice} "
                           f"| 评分{result.ai_result.sentiment_score}")

                # 核心结论（截断）
                if result.ai_result.analysis_summary:
                    summary = result.ai_result.analysis_summary[:60] + "..." if len(result.ai_result.analysis_summary) > 60 else result.ai_result.analysis_summary
        ...
        # 检查长度
        if len(content) > max_length:
            logger.warning(f"选股报告超长({len(content)}字符)，截断")
    ...
    def send_screening_report(
        ...
        try:
            # 生成报告
            report = self.generate_screening_report(screening_results, report_date=report_date)
            ...
            if save_to_file:
                ...
                filename = f"screening_report_{date_str}.md"
                filepath = self.save_report_to_file(report, filename)
                logger.info(f"选股报告已保存: {filepath}")

            # 推送通知
            if self.is_available():
                # 生成精简版用于推送
                wechat_content = self.generate_wechat_screening_report(screening_results, report_date=report_date)
                success = self.send(wechat_content)
                if success:
