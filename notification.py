...
class NotificationService:
    ...
    def __init__(self):
        ...
        self._custom_webhook_bearer_token = getattr(
    ...
    def _is_pushover_configured(self) -> bool:
        """检查 Pushover 配置是否完整"""
        return bool(
            self._pushover_config["user_key"] and self._pushover_config["api_token"]
        )
    ...
    def generate_daily_report(
        ...
        buy_count = sum(
            1
            for r in results
            if r.operation_advice
            in ["买入", "加仓", "强烈买入", "突破买入", "回踩确认"]
        )
        sell_count = sum(
    ...
    def _get_signal_level(self, result: AnalysisResult) -> tuple:
        ...
        if advice in ["强烈买入", "突破买入"] or score >= 80:
            return ("强烈买入", "💚", "强买")
        elif advice in ["买入", "加仓", "回踩确认"] or score >= 65:
            return ("买入", "🟢", "买入")
    ...
    def _count_signal_summary(self, results: List[AnalysisResult]) -> Dict[str, int]:
        ...
        for result in results:
            ...
            if signal_text in ["强烈买入", "突破买入", "买入", "回踩确认"]:
                counts["buy"] += 1
    ...
    def _send_feishu_message(self, content: str) -> bool:
        ...
        card_payload = {
            ...
            "card": {
                ...
                "header": {
                    "title": {"tag": "plain_text", "content": "A股智能分析报告"}
                },
                "elements": [
    ...
    def _send_pushover_chunked(
        ...
        for i, chunk in enumerate(chunks):
            # 添加分页标记到标题
            chunk_title = (
                f"{title} ({i + 1}/{total_chunks})" if total_chunks > 1 else title
            )
    ...
    def _send_dingtalk_chunked(
        self, url: str, content: str, max_bytes: int = 20000
    ) -> bool:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if body_bytes > max_bytes:
                ...
                payload["markdown"]["text"] = self._truncate_to_bytes(
